...
resource "aws_wafv2_web_acl" "shomotsu_waf" {
  name        = "ShomotsuWAF"
  description = "WAF for Shomotsu"
  scope       = "CLOUDFRONT"
  ...
  tags = var.shomotsu_tags
 
  visibility_config {
    cloudwatch_metrics_enabled = false
    metric_name                = "ShomotsuWAFMetric"
    sampled_requests_enabled   = false
  }
}
...
resource "aws_s3_bucket" "shomotsu_log_bucket" {
  bucket = "aws-waf-logs-${var.shomotsu_bucket_name}"
...
resource "aws_wafv2_web_acl_logging_configuration" "shomotsu_log_config" {
  log_destination_configs = ["${aws_s3_bucket.shomotsu_log_bucket.arn}"]
  resource_arn = aws_wafv2_web_acl.shomotsu_waf.arn
  provider = aws.us-east-1
